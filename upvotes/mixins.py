...
class UpvoteMixin:
    ...
    """
    upvote_serializers = {'upvote_list': UpvoteSerializer,
                          'upvote': UpvoteSerializer,
                          }

    @decorators.action(url_path='upvote', detail='upvote', methods=('POST',), )
    ...
    @decorators.action(url_path='upvotes', detail='upvote-list', methods=('GET',), )
    def upvote_list(self, request, pk):
        upvotes = Upvote.objects.filter(post_id=pk)
        serializer = self.get_serializer_class()(upvotes, many=True, context={'request': request})
